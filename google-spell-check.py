...
class GoogleSpellCheckCommand(sublime_plugin.TextCommand):
	...
	def correct(self, text):
		...
		if start_google_suggestion==-1:
			return text
		else:
			# finds dimesnions of google duggestion
			end_google_sug = html_result.find('</a>',start_google_suggestion)

			# finds dimesnions of suggested text
			start_sug_text = html_result.find("&amp;q=",start_google_suggestion,end_google_sug)+7
			end_sug_text = html_result.find("&amp;spell",start_sug_text,end_google_sug)
			sug_text = html_result[start_sug_text:end_sug_text]
			match = urllib.parse.unquote(sug_text.replace('+',' '))
